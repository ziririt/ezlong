 ...
 *
 * 운영자 결정(2026-07-28): "글로벌 버전에서 알라딘은 완전히 뺀다."
...
(function (root, factory) {
  ...
  /**
   * 로케일 → 기본 지역. 브라우저가 국가를 알려주지 않을 때(언어 태그에 지역이
   * 없는 흔한 경우) 어느 아마존으로 보낼지 정한다. 지역이 잡히면 그쪽이 항상
   * 우선이다 — es-MX 사용자는 스페인이 아니라 멕시코 아마존으로 간다.
   * 2026-08-09 운영 지침로 신설(그전에는 영어가 아니면 링크 자체가 없었다).
   * zh 를 com 으로 두는 이유: 아마존 중국은 도서 판매를 접어서 보낼 곳이 없다.
   */
  ...
  function resolve(quote, opts) {
    try {
      ...
      // ── 한국어 외의 모든 언어: 아마존 검색 ──
      // 2026-08-09 운영 지침로 방침 변경. 그전(7/29)에는 영어가 아니면 링크를
      // 아예 만들지 않았다 — "영문 검색어를 던져봐야 자기 언어 판본을 못
      // 찾아준다"는 판단이었다. 지금은 반대로 본다: 아마존에 그 책이 있다면
      // 일본·스페인·브라질 이용자도 자기 나라 아마존에서 찾아볼 수 있어야 한다.
      // 어느 아마존으로 보낼지는 아래에서 지역(우선) → 로케일 기본값 순으로
      // 정한다. 검색어에 한글이 섞인 책(한국 도서)은 그 아래에서 여전히 걸러진다.
      ...
      // 2026-07-29 운영자 확정: "영어는 미국 아마존".
      // 지역별 라우팅(영국 사용자 → .co.uk)을 쓰지 않는다. 이유가 둘이다.
      //   1. 제휴 태그는 나라별로 따로 발급받아야 하고, 지금 가지고 있는(가질)
      //      태그는 미국(.com) 하나다. 영국 링크에 미국 태그를 붙이면 수수료가
      //      아예 안 잡힌다 — 즉 지역 라우팅은 지금 구조에서 수익을 0으로 만든다.
      //   2. .com 은 배송망이 가장 넓어, 최악의 경우에도 "못 산다"가 되진 않는다.
      // 트레이드오프: 영국·호주 사용자는 자국 아마존보다 배송이 느리고 비쌀 수
      // 있다. 나라별 태그를 여러 개 발급받는 날 opts.region 을 되살리면 된다
      // (detectRegion·REGION_TO_TLD 코드는 그때를 위해 지우지 않고 남겨둔다).
      var region = opts.region || LOCALE_TO_REGION[locale] || DEFAULT_REGION;
      return {
        kind: "amazon",
