...
- **1차 모델: `gemini-2.0-flash`** — GA 정식 출시, 안정적, 논리 오류 적음
- **폴백 모델: `gemini-2.5-flash-lite`** — 프리뷰(불안정), 503 빈발, 논리 오류 확인됨
- `gemini-2.5-flash` 사용 금지 (비용 문제)
- `thinkingConfig` 사용 금지
- **변경 배경:** gemini-2.5-flash-lite가 원유 가격 하락을 "인플레이션 우려"로 잘못 분류하는 등 매크로 인과관계 오류 반복 확인 (2026-06-25). GA 안정 모델로 1차 전환.
