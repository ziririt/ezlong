...
## 80. 긴 대시(-)를 쓰지 않는다 - 모든 화면 문구 (2026-08-27, 성동님 지시)

**지시 원문:** "'1배수 칸은 채운 채로 유지 - 레버리지 검토는 반등 관문'와 같은 표현에서
' - '는 지양하고 ' - '으로 해라. 모든 텍스트에서 이렇게 해라."
(원문의 앞쪽 기호는 em dash, 뒤쪽은 하이픈. 확인 결과 **같은 줄에 ' - '**로 확정.)

**왜.** em dash는 한국어 본문에서 폭이 넓어 시선을 끊고, 복사해 다른 글에 붙이면 폰트에
따라 깨져 보인다. 34항이 표를 금지한 이유("복붙 시 깨짐")와 같은 계열의 문제다.
하이픈은 어디서나 같은 모양으로 붙는다.

**세 겹으로 막는다.**
1. **공용 모듈 `scripts/ez_text.py`** - `fix_dash(text)` / `scrub(obj)`.
   - 낱말 사이: `(?<=\S)[ \t]*—[ \t]*(?=\S)` → `' - '`
   - 줄머리: `(^|\n)([ \t]*)—[ \t]*` → `\1\2- ` (**들여쓰기를 살린다**)
   - 남은 것: `-`
   - `python3 scripts/ez_text.py <파일...>`로 일괄 정리, `--selftest`로 8건 자체 시험.
2. **저장 직전 훑기** - 생성 스크립트가 JSON을 쓰기 전에 `_ez_scrub()`을 통과시킨다.
   `fetch-market-scorecard.py` · `fetch-today-chart.py` · `generate-swing-view.py` ·
   `fetch-model-portfolio.py` · `build-weekly-risk.py`, 그리고 JS 쪽
   `generate-chart-analysis.js`의 `ezScrub()`(같은 규칙을 JS로 옮긴 것).
   **모듈이 없어도 본 기능은 죽지 않는다**(try/except 폴백).
3. **프롬프트** - 스코어카드·스윙 데스크·차트분석 세 곳에 "긴 대시를 쓰지 마라" 규칙.
   나쁜 예/좋은 예를 나란히 뒀다.

**일괄 정리(2026-08-27).** 265개 파일. HTML·공용 JS·데이터 JSON 전부.
**엔티티도 같이 잡아야 한다** - `&mdash;`(97개 파일)가 남아 있어 화면에는 계속 보였다.
실문자만 훑고 끝냈다가 렌더 검증에서 걸렸다. **문자 치환은 표기 형태를 전부 세어야 한다.**

**손대지 않은 곳:** `time/`(앱 전용 관리 영역, 38항) · `skybluenote/`(별개 프로젝트) ·
`scripts/*.py|js`의 주석과 코드(표시 문자열은 2단계가 저장 시점에 처리한다) ·
`*.md` 문서(내부 문서라 화면에 안 나간다).

**검증:** 데이터 JSON 288개 파싱 실패 0 · 공용 JS 문법 전부 통과 · 사이트맵 XML 정상 ·
주요 15페이지(한국어 12 + en·ja·zh) 렌더 결과 화면 텍스트에 em dash **0건**,
가로 스크롤 없음, JS 오류 0.

**남겨둔 것:** en dash(-)는 이번 지시 범위가 아니라 두었다. 37개 파일에서 주로
연도 범위("2023-2025")에 쓰인다. 정리하려면 같은 도구에 규칙 한 줄만 더하면 된다.

**교훈:** "모든 텍스트에서"라는 지시는 **저장소 전체 훑기 + 앞으로의 보증** 두 가지를
같이 해야 지켜진다. 일괄 치환만 하면 다음 생성 때 되돌아오고, 프롬프트만 고치면
모델이 가끔 어긴다. 51항 이래 반복된 그 구조를 표기 규칙에도 그대로 적용했다.
