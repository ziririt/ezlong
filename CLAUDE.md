...
- **1번(이 세션)** — 클라우드 샌드박스에서 작업하고, 샌드박스는 GitHub push가 막혀
  ...
  상세는 HANDOFF.md 7-2절.
- **2번(`ziririt@aladin.co.kr`)** — 같은 맥북의 **다른 사용자 계정**이다. 홈 디렉터리가
  달라 `~/Developer/ezlong`이 **그 계정에는 없을 수 있다.** 저장소를 새로 clone할지,
  1번 계정 폴더를 공유할지(권한 문제), GitHub 인증이 그 계정에 있는지 —
  **이 셋은 실제로 확인되기 전까지 가정하지 않는다.** `docs/PENDING.md` P-002 참조.
