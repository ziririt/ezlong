...
- **메인 `#apps` 섹션**: 유틸리티 도구 바로 아래. 도구 목록 다음이 이 사이트에서
  "내가 만든 것"을 잇는 자연스러운 자리다. 카드 두 장, 둘 다 `/app/` 로 간다.
...
**교훈:** 페이지를 만들고 사이트맵에 올리는 것까지는 '만들기'다. **사이트 안에서
그 페이지로 가는 길을 내는 것까지가 '내보내기'다.** 만들었는데 아무도 못 가는 페이지는
검색엔진에만 존재한다. 새 페이지를 만들 때 메인·푸터·내비 세 자리 중 어디에 걸지를
같이 정하는 것을 기본으로 삼는다.
