...
- `_price_result_name(name)` — 이름이 결과 서술인지 판정. **두 갈래만** 잡는다:
  (1) '주가·시총'이 주어인 등락, (2) 개별 기업 이름(`_RPT_COMPANY_GROUPS`) + 등락어.
  '반도체 섹터 강세' 같은 시장 단위 서술은 체크 11이 맡는다 — 여기서 같이 잡으면
  집행 범위가 갑자기 넓어져 오탐의 대가가 커진다(52항의 교훈).
...
**시험:** 20문장(정상 통과 13건 — 벨웨더 실적·금리·유가·지정학·달러) 전부 통과.
저장된 카드 10장에 소급하니 이번 사고 1건만 잡혔다(오탐 0).

**소급:** 위반 카드 1장을 삭제하고 판정 원장에서도 뺐다. 재료 하나를 걷어내면
