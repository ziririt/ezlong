...
거래 특성 확인). **조치:** `TVC:SPX`/`TVC:IXIC`/`TVC:DJI`(TradingView 자체 무료 인덱스 피드,
"SP:SPX/DJ:DJI와 차트 동일, 누구나 이용 가능"으로 웹 검색 확인)로 교체 — 실제 NYSE 연동
지수라 다른 차트와 동일한 세션 캘린더를 따를 것으로 기대. 부가로 NDX를 TVC:IXIC(진짜 종합
지수)로 맞춰 상세 차트까지 정의 일치 완성. `SYMBOL_NAMES['NDX']`도 "Nasdaq 100 Index" →
"Nasdaq Composite Index"로 정정. **검증 한계:** TradingView 위젯은 브라우저에서 실제
렌더링해야 확인 가능한 클라이언트 라이브러리라 샌드박스에서 직접 검증 불가 — 배포 후
반드시 브라우저에서 지수 3개 상세 차트 1D/1분봉 진입해 시작 시각이 다른 차트와 같은지
육안 확인 필요.
...
**규칙:**
- Baseline 스타일로 되돌리지 않는다 — 퍼센트 기준선이 실제 등락 방향과 무관해 오해를
  유발한다는 게 확인된 사실이다.
...
**재시도 상한선 관련:** 18항 규칙상 같은 증상 3회 실패 시 중단·보고 대상이다. 이번이
"차트 색상/모양 불일치" 증상의 2번째 수정 시도(1차 Baseline 기본값 실패 → 2차 Area
단색 실패 → 3차 Baseline+근사기준선). 3차도 배포 후 육안 확인에서 여전히 이상하면,
더 시도하지 말고 **캔들스틱(`style:'1'`)로 전환**을 최종 대안으로 제시할 것 — 캔들은
봉 하나하나가 자기 시가 대비 등락으로 색이 정해지는 TradingView 네이티브 기본 동작이라
퍼센트/기준선 추정이 전혀 필요 없어 100% 신뢰 가능하다(대신 1분봉 다수라 선 차트보다
시각적으로 복잡해 보이는 트레이드오프).
