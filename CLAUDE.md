...
## 43. 소사(A Brief History) — 손으로 쓴 것과 기계가 얹은 것 (2026-08-09)

`/brief-history.html`. 한 파일(`data/brief-history.json`)에 성격이 다른 두 가지가
섞여 있다. `source` 로 구분하고, 서로의 영역을 넘지 않는다.

- **손으로 쓴 소사** — `external_knowledge` / `own_article`, importance 2·3.
...
### 날짜 매핑 — 발행일이 아니라 '다룬 장의 날짜'

아침 시황은 전날 미국장을 쓴다. 콘텐츠 id 앞 12자리가 발행 시각(KST)이므로,
그걸 뉴욕 시간으로 옮겨 **그 시점에 이미 마감된 가장 최근 정규장**을 고른다
(16:00 ET 이전이면 하루 앞으로). 주말·공휴일은 거래일 달력으로 스냅한다.
실측 대조: 3/24 08:10 발행 글 → 3/23 장, 본문의 `S&P500 +1.15%` 와 계산값
`SPY +1.08%` 일치.
...
기록은 거의 매 거래일에 있다. 전부 찍으면 시간축 위에 띠 하나가 생겨 정보가
아니라 잡음이 된다 → **보이는 봉이 170개 이하일 때만** 기록 점을 찍는다
(소사는 항상). 달력·목록에는 항상 나온다.
