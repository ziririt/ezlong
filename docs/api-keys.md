...
## 매일 점검 예약 작업 (클라우드, 성동님 클로드 계정)

매일 09:00 KST에 클라우드 세션이 하나 떠서 이 대장과 데이터 시각을 읽고 판단한다.
- 파이프라인 정지(스코어카드 6시간, 차트분석 30시간 넘게 멈춤): 즉시 알림
- 키 만료 3일 이내: 매일 알림. 4~21일: 월요일에만. 만료일 미기재: 월요일에만
- 이상 없음: 한 줄로 끝
이어받는 사람은 같은 내용을 본인 계정에 다시 만든다. 읽기 전용이고 커밋·강제 실행을 하지 않는다.
