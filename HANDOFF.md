...
### 4-0. 88항 거래일 캘린더 - 브랜치에 미완으로 살아 있다 (최우선)

**`wip/88-calendar-truth` 브랜치, 커밋 `177356b`. `main`에는 없다.**
2026-09-07 노동절에 화면이 '오늘 장(9월 7일) 장중'이라고 쓴 사고에서 시작됐고,
작업하던 세션이 크레딧 소진으로 끊겼다. 무엇이 되어 있고 무엇이 남았는지는
...
재발한다. **캘린더와 단일 진실값(원칙 3·4)을 먼저 끝내고, 그다음 상위 판정 지배와
출력 검문(원칙 2·5)이다.** 점수식 방향 항(원칙 7)은 백테스트 후 성동님 확인.
