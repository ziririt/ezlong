...
박는다. 지금 90항까지 있다. **이 번호 체계가 이 프로젝트의 척추다.**
...
### 4-0. 88항 - 원칙 3·4·5·9는 배포됐고 7·8이 남았다 (89·90항은 끝)
...
89항(시장 날짜의 상대어 금지)과 90항(검문은 모든 출구에)은 2026-09-09에 **끝내서
배포했다.** 88항 원칙 5·9(출력 검문·종목 카드 동일 검문)도 그때 같이 닫혔다.
**남은 것은 원칙 7(점수식 방향 항)과 원칙 8(기저율 병기) 둘뿐이다.**
원칙 7은 SOXX 5년 백테스트 결과를 성동님이 보고 결정할 사안이라 코드부터 짜면 안 된다.
...
- **원칙 7(점수식 방향 항).** 점수가 방향을 못 담아 "매수 74점인데 하락 추세"가
  나온다. SOXX 5년 백테스트를 먼저 돌리고 **결과를 성동님께 보여 결정**한다.
  코드부터 짜면 안 된다: 점수식은 이 사이트의 심장이라 되돌리기가 비싸다.
- **원칙 8(기저율 병기).** "매수점수 74"만으로는 그 숫자가 좋은지 알 수 없다.
  같은 조건에서 과거 5거래일 뒤 상승 확률이 몇 %였는지를 옆에 붙인다.
  TimesFM 채점(4-4)과 자료가 겹치니 같이 하는 편이 싸다.
