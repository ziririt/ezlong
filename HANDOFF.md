...
- `atmr-dashboard.html`: 스윙 시그널 대시보드. 이 사이트의 심장. 약 42만 자.
  **탭이 둘이다(2026-09-13 통합).** ① 스윙 시그널 - 나스닥100 QQQ·S&P500 VOO·반도체
  SOXX 로 **미국 시장**을 판정하고, 그 아래에 3-3-4 분할·레버리지 단계·Gear 설명이
  이어진다. ② TOP9 집중분석 - 빅테크 9종을 종목별로 본다.
  옛 '스윙 전략' 탭은 ①로 흡수했다. 옛 해시 `#swing-strategy` 는 ①을 연다.
  맨 위 '반등·반락 판별' 패널은 `swing-reversal.js` + `swing-reversal.css` 가 그린다 -
  세 지수 중 **둘 이상이 같은 방향일 때만** 판정하고, 하나만 남으면 판정하지 않는다.
  시험은 `node scripts/swing-reversal-selftest.js`(185건).
...
### 4-13. 확인 대기 정리 (2026-09-12 저녁)

- **열린 항목은 P-005(토큰 폐기) 하나뿐이다.** 20:00 KST 복귀 예약으로 성동님께 위험 설명과
  함께 다시 올린다(`trig_01CBBBqu68BzLXtj8bx3zgkE`).
- P-002(2번 계정 환경)·P-003(총괄 에이전트)·P-004(예약 묶임)·P-006(규칙 파일 개수) 해소.
  P-003 은 **다른 주자가 `_ops` 로 먼저 해결해 둔 것을 내가 못 닫고 있었다** — 그 문서를
  읽고 피드백까지 썼는데도. 확인은 성동님 답 말고 다른 주자의 작업으로도 풀린다는 것을
  CLAUDE.md 2절에 박아 두었다.
...
- **규칙 다이어트 분류 초안 완료.** 94개 항 줄 수 실측 - 상시 후보 1,312 / 코드가
  집행 1,249 / 기능별 2,352. 파일 개수는 성동님이 **① 세 파일**로 확정(2026-09-12) - `CLAUDE.md`(상시본 576줄) +
  `docs/RULES-REF.md`(기능별 42항) + `docs/INCIDENTS.md`(사고 기록 52항). 같은 날 이관 완료.
  94항이 원문 그대로 한 번씩 옮겨졌고 색인 줄 수까지 대조했다. 블라인드 감사 2회에서
  나온 지적 12건 전부 반영. **항 번호는 절대 바꾸지 않는다.**
...
### 4-6. 확인이 필요한 수치: ISA 한도 - 끝났다 (2026-09-13)

`tax-account-simulator.html` 의 2,000만원·비과세 200만원은 **현행이 맞다.**
2차 자료들이 말하던 "연 4,000만원·비과세 500만원"은 8월 개편안이고 9월 1일
최종안에서 철회됐다. 세율·한도는 `ez-tax-rules.js` 로 옮겼다(`ASOF`·`SOURCE` 포함).
12월 국회에서 생산적금융 ISA 가 통과되는지는 `docs/PENDING.md` P-007 로 열어 뒀다.
**교훈: 제도·세법은 1차 자료로만 판정한다. 확인 못 했으면 기울지 않는다**(CLAUDE.md 2절).

### 4-14. 2026-09-13에 끝난 것

하루에 여덟 덩이를 배포했다. 전부 대피소 `stable-` + 체크포인트 `cp-` 태그 쌍이 있다.

- 계산기 넷 × 6언어(24파일)의 이율 환산을 `ez-finance.js` 하나로 통일 - `cp-20260913-calc-unify`
- 실패를 정상으로 읽던 초기값 제거(마켓 사이클) - `cp-20260913-fail-loud`
- 매매 레벨 검문(ATR 배수·손익비 1.5·방향 정합) - `cp-20260913-level-gate`
- 세율·한도를 `ez-tax-rules.js` 로 - `cp-20260913-tax-rules`
- 포트폴리오 2배 레버리지 토글 제거(국내 증거금 규제) - `cp-20260913-no-leverage`
- 반등·반락 패널 신설 - `cp-20260913-reversal`
- **스윙 시그널 통합 + 세 지수 종합 시장 판정** - `cp-20260913-swing-merge`
...
- **메인 프리미엄 콘텐츠 자리를 인사이트 타임스로 교체** - `cp-20260913-insightimes`
  ...
  **지면(insightimes.com)을 먼저 읽는다** - 편집이 배치한 순서와 썸네일·분류·날짜가
  ...
  어디서 왔는지는 JSON 의 `via`(page/rss) 로 확인한다.
  내린 글은 `scripts/insightimes-exclude.json` 으로 거른다 - 지면에서 내려도
  주소와 RSS 항목은 한동안 살아 있어 자동으로는 못 걸러진다(실측: HEAD 200).
...
- 스윙 시그널 탭 머리 순서 확정 - `cp-20260913-tab-order`
  **① 볼카운터 ② 최근 흐름 ③ 반등·반락 판별.** 요약이 먼저고 근거가 뒤다.
  ...
  반등 패널의 '스윙 시그널 · 베타' 소제목도 제거 - 히어로·탭 이름과 세 번 겹쳤다.

**남은 것:** 배치 06 백테스트 재건(별도 세션. 통과 기준 셋 - SPY-IVV 상관 >0.98,
SPY 최대낙폭 -50~-58%, SPY-QQQ 0.85~0.95). 14px 미만 텍스트 정리(은퇴 46건·복리 31건).
`docs/PENDING.md` P-007~P-009.
...
> **2026-09-12부터: 맥에서 도는 git 명령마다 앞에 `export OPS_RUNNER='ezlong-주자'` 를 붙인다.**
> `_ops` 공용 훅이 여섯 저장소에 물렸다. `do shell script` 는 매번 새 비로그인 셸이라
> 환경변수가 안 딸려온다(실측 확인). 잠금이 없으면 지금은 통과하지만, 남이 ezlong 을
> 잡은 순간 **내가 임자여도 이름이 비어 있어 통과하지 못한다.**
> 그리고 `git am` 도 이제 열쇠 검사를 탄다(`pre-applypatch` — 내가 만들었다).
> 패치에 열쇠가 섞여 있으면 **맥에서 `git am` 이 실패한다.** 우회하지 말고 패치를 고친다.
