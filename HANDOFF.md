...
더 깊은 규칙은 `CLAUDE.md`(87항)가 본체다. 이 문서와 충돌하면 `CLAUDE.md`가 이긴다.
...
박는다. 지금 89항까지 있다. **이 번호 체계가 이 프로젝트의 척추다.**
...
### 2-3. 최근에 끝난 일 (2026-08-26 ~ 09-08)

`CLAUDE.md` 75항부터 89항까지가 이 기간이다.
...
### 4-0. 88항 - 원칙 3·4는 배포됐고 5·7·8·9가 남았다 (89항은 끝)
...
89항(시장 날짜의 상대어 금지)은 2026-09-08에 **끝내서 배포했다.** 남은 잔가지는
`CLAUDE.md` 89-3에 적어 뒀다: 킹 카드 배지 '오늘의 판단'이 아직 정적 문구다.
...
지금 화면에 **틀린 값이 떠 있다.** 보유 기간이 한 화면에 셋(15일째·31거래일째·
30거래일째)이고, 매수 점수가 카드 60·본문 59로 갈리고, 최상단이 관망인데 3배수 카드는
진입 가능이라고 한다. **원칙 3·4는 끝났다(2026-09-08 배포)** — 다음 미국 공휴일
(11/26 추수감사절)에 '장중' 오류가 재발하지 않는다. 다음은 출력 검문(원칙 5) —
RSI 40 이상에 '과매도', 진입 기준 미달인데 '매수', 손익비 1 미만 플랜 표시를
코드가 막는다. 점수식 방향 항(원칙 7)은 백테스트 후 성동님 확인.
