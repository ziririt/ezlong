...
최종 갱신: 2026-09-12(토) 09:18 KST
기준 커밋: `main` 최신(`8a28c99d` 이후)
...
>
> **배포 경로는 주자마다 다르다.** 1번은 7-2절 그대로. 2번은 홈 디렉터리가 달라
> `~/Developer/ezlong` 이 없을 수 있다(`docs/PENDING.md` P-002 - 아직 미확인).
...
### 4-10. 주자 교대 준비 (97항, 2026-09-12)

- **`docs/PENDING.md` P-002 - 2번 주자(`ziririt@aladin.co.kr`) 환경 미확인.**
  그 계정에 저장소가 있는지, GitHub 인증이 있는지, 1번 계정 폴더를 공유할 수 있는지
  셋 다 확인된 적이 없다. **1번 한도가 찬 뒤에 알게 되면 그 자리에서 막힌다.**
  성동님이 그 계정으로 Cowork 를 한 번 열어 보시면 답이 나온다.
- 확인되면 이 문서 **7-2절에 2번 주자용 절차**를 적는다. 3번(supergrok)용으로는
  "복사해서 바로 붙일 명령" 형태의 절이 따로 필요하다.
...
### 0순위(성동님 몫). 2번 주자 환경 확인 - P-002

`ziririt@aladin.co.kr` 계정으로 Cowork 를 한 번 열어 저장소에 닿는지 확인한다.
**한도가 찬 뒤에 확인하면 늦다.** 확인되면 7-2절에 그 계정용 배포 절차를 적는다.
