...
// 88항: 거래일 캘린더(단일 출처 data/nyse-calendar.json). 실패하면 요일 판정으로 폴백.
let ezCal = null;
...
// 최근 판단 컨텍스트: 거래일별 마지막 판단만 추려 최대 4거래일치 반환.
// (88항 정정) "파이프라인이 거래일에만 돈다"는 가정은 틀렸다: 감시견은 휴장일에도 돌고, KST 날짜는 ET 거래일과 어긋난다.
// d 가 이제 ET 거래일이므로 여기서 날짜별로 묶으면 그대로 거래일별이 된다.
...
// 88항: 원장의 날짜 d 는 '판단이 속한 미국 거래일(ET)'이다. 종전엔 KST 날짜여서 한 거래일이 두 날짜로 갈라지고
// (장중 01시 KST 와 마감 06시 KST), 휴장일(9/7 노동절)에 돈 판단이 새 거래일처럼 쌓였다. kst 는 표시용으로 남긴다.
function ledgerDay() {
