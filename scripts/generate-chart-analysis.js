...
/* 80항 - 화면 문구의 문장 부호. 파이썬 쪽 scripts/ez_text.py 와 같은 규칙이다.
   ...
   · 이미 하이픈이 된 구분자도 콜론으로. 단 음수·범위·연산·수식은 건드리지 않는다 */
function ezFixDash(t) {
  ...
  if (s.indexOf(' - ') >= 0 && !/[()].*\//.test(s)) {
    s = s.replace(/([가-힣A-Za-z%)\]]) - (?=[가-힣A-Za-z])/g, '$1: ')
         .replace(/([가-힣]) - (?=\d)/g, '$1: ');
  }
  return s;
}
...
const TICKERS_US_STOCKS = [
  ...
  // 2026-09-22 추가 — 심플 주가 시총 상위권인데 차트분석에 없던 종목(오너 요청).
  { symbol:'SPCX',  name:'스페이스X',         type:'stock', context:'발사체와 스타링크 위성통신이 핵심 사업인 우주기업. 2026년 6월 12일 나스닥 상장이라 거래 이력이 짧아 SMA100·SMA200 과 주봉 지표는 아직 계산되지 않습니다. 장기 추세 판단을 단정하지 마십시오.' },
  ...
  { symbol:'ORCL',  name:'오라클',            type:'stock', context:'데이터베이스·기업용 소프트웨어 강자. OCI 클라우드의 AI 데이터센터 수주가 주가를 좌우하며, 대규모 설비투자와 부채가 변수입니다.' },
  ...
  { symbol:'ARM',   name:'Arm',               type:'stock', context:'스마트폰·데이터센터 CPU 설계 IP 를 라이선스하는 기업. 로열티 구조라 AI 서버용 Arm CPU 채택 확대가 성장을 좌우합니다.' },
...
` : '';
  ...
  // ── 시장 전체 스윙 판단 컨텍스트 (2026-07-31 신설) — 미국 시장 주식/ETF에만 주입 ────
  ...
  const chiefSection = (() => {
    ...
    try {
      ...
      return `
[시장 전체 스윙 판단 맥락 (v2) (${sv.generatedAtKST} 기준)]
오늘 시장 전체 스탠스: ${sv.comp.stanceLabel}${sv.flow ? ` | 판단 흐름: ${sv.flow}` : ''}
${first}
맥락 규칙 (반드시 지켜라):
- 위는 시장 전체(QQQ·VOO·SOXX 종합)에 대한 종합 판단이다. 이 종목의 기술적 판독이 같은 방향이면 그 정합을 한 문장으로 언급하라.
- 어긋나면 어긋난다는 사실과 "이 종목만의" 기술적 이유를 reason에 반드시 명시하라. 모순을 숨기지 마라.
- 단, 시장 전체 판단에 끌려가서 이 종목의 지표와 다른 판단을 내리지는 마라. 판단 근거는 항상 이 종목의 오늘 지표다.
`;
    } catch (e) { return ''; }
  })();
...
- 문장 부호(80항): 긴 대시를 쓰지 마라. 항목과 설명·부연을 잇는 자리에는 콜론 ':'을 쓴다.
  하이픈은 마이너스와 구분이 안 되므로 음수·범위·줄머리 목록에만 쓴다.
...
- 짧게 자르라는 뜻이 아니다. 수치·조건·해석을 다 담되 말끝만 명사로 맺어라.
  나쁜 예: 'RSI가 38.1까지 상승했고 MACD도 개선세를 보이며 -2.7561로 올라왔다'
  좋은 예: 'RSI 34.9→38.1 상승으로 과매도 구간 탈출, MACD 히스토그램 -2.7561로 개선'
- 영어 필드(...En)는 이 규칙에서 제외 — 자연스러운 영어 완결 문장으로 쓴다.
- [자기 평가 금지] 과거 판단이 시장에 뒤처졌더라도 반성문·사과·자책을 쓰지 마라.
  금지어 — '실책', '인정한다', '너무 보수적이었다', '더 실었어야', '놓쳤다', '솔직한 복기'.
  자랑도 같은 무게로 금지 — '예측대로', '정확히 맞혔다'. 숫자는 그대로 적되(성적은 숨기지
  않는다) 문장은 지금 시장 상태와 다음 조건으로 쓴다. 자책도 자랑도 독자에게는 정보가 아니다.
...
다음 JSON만 반환하라. 다른 텍스트는 절대 붙이지 마라:
{
  ...
  "reason": "판단 근거 2~3줄 (숫자 포함, 가장 강한 근거만). 문체 규격대로 명사형 종결 — '~올라왔다', '~필요하다' 같은 해라체 금지",
  ...
  "keyPoints": ["핵심 포인트 1 (숫자 포함, 명사형 종결)", "핵심 포인트 2 (숫자 포함, 명사형 종결)", "핵심 포인트 3 (숫자 포함, 명사형 종결)"],
  "keyPointsEn": ["keyPoints[0]의 영어 버전", "keyPoints[1]의 영어 버전", "keyPoints[2]의 영어 버전"],
  "riskNote": "기술적 리스크 한 문장, 명사형 종결",
  "riskNoteEn": "riskNote의 자연스러운 영어 버전",
  "continuity": "${historyLines ? "직전 판단 기록 대비 오늘의 흐름 1~2문장. 문체 규격대로 명사형 종결. 다만 반드시 '지금까지의 상태'로 읽혀야 한다 — 'N일 연속 관망 유지'처럼 쓰면 앞으로 관망하라는 권고문으로 오독되므로, 주어에 반드시 '판독' 또는 '판정'을 넣어라. 판단 유지면 '오늘까지 3거래일 연속 관망 판독 지속' 형태. 판단 전환이면 전환 이유(지표 변화)를 숫자와 함께 명시, 예: '어제 관망 → 오늘 매수 전환, RSI 34.9→38.1 상승과 MACD 히스토그램 -2.7561 개선이 근거'" : "기록 없음 — 첫 판단"}",
