...
다음 JSON만 반환하라. 다른 텍스트는 절대 붙이지 마라:
{
  ...
  "stopLoss": 손절 기준가 숫자,
  "narrative": "아래 4개 섹션 구조로 작성하라. 섹션 제목은 대괄호로 감싸라([추세 위치] 형태). 각 항목은 · 으로 시작. 문장 끝은 서술어 없이 명사형으로 끝맺기. '~할 수 있다', '~가능성', '~예상', '~전망' 절대 금지. 숫자는 반드시 포함. 항목당 한 줄.\n\n[추세 위치]\n· 현재가($숫자) vs SMA5/20/50/100/200 위치 — 단기·중장기 추세 진단\n· 52주 위치(%숫자) 언급 + 현 가격대 역사적 맥락\n[모멘텀 지표]\n· RSI 궤적 (이전값→현재값) + 과매수/과매도/중립 판단\n· MACD 히스토그램 궤적 (이전값→현재값) + 개선/악화 진단\n· 볼린저밴드 위치 (상단/중단/하단) + 밴드폭 해석\n[지지·저항 구조]\n· 스윙 지지선 ($숫자) + 근거 (어떤 지표 기반인지)\n· 스윙 저항선 ($숫자) + 돌파 시 의미\n· 5일 고가/저가 vs 20일 고가/저가 패턴 (Higher Low / Lower High / 횡보)\n[거래량·결론]\n· 거래량 비율 (5일 평균 대비 숫자배) + 힘 있는 움직임인지 판단\n· 현 구간 진단 (축적/분배/추세/횡보) + 진입·관망 조건 명시",
