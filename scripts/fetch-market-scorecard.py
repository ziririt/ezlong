...
- **개별 기업 뉴스에 점수를 싣지 마라 — 카테고리 라벨로 피해 갈 수 없다.** 한 기업의
  ...
  그 뉴스가 시장에 옮겨붙은 것을 말하려면 **파급 대상을 이름이나 설명에 명시하라** —
  '기술주 전반', '반도체 섹터', '빅테크', '밸류체인'처럼 번져 나간 대상을 적는다.
  좋은 예: name '구글 광고 사업 분할 기각', desc '주요 기술 기업 규제 리스크 완화'
  (판결 하나가 빅테크 전체의 규제 전망을 바꾼다 — 시장 재료다).
  나쁜 예: name '엔비디아 AI 관련 투자 기대', desc '엔비디아의 노키아 투자 가치 상승'
  (한 회사의 지분 가치 얘기로 끝난다 — 시장이 움직일 이유가 없다).
  '시장 심리 개선' 같은 말은 파급 대상이 아니다. 무엇에 번졌는지를 적어라.
...
# 파급 면제(2026-09-03 같은 날 교정) — 첫 판이 너무 좁았다. 실적만 예외로 뒀더니
# '구글 광고 사업 분할 기각'(빅테크 전반의 반독점 판례)이 개별 기업 뉴스로 걷어차여
# 긍정 칸이 통째로 비고 카드가 안 나갔다. 51항 G4의 취지는 '한 기업의 지역 판매·
# 등급 조정' 같은 뉴스를 막는 것이지, 업종 전체에 번지는 규제·판결을 막는 게 아니다.
# 그 뉴스가 시장에 옮겨붙었다면 **파급 대상**이 이름이나 설명에 적혀 있어야 한다 —
# '시장 심리 개선' 같은 헐거운 말은 대상이 아니므로 '시장'은 목록에 넣지 않는다.
_G4_SPILLOVER   = (r'섹터|업종|전반|반도체|기술주|성장주|가치주|빅테크|기술\s*기업|'
                   r'관련주|테마주|나스닥|다우|러셀|S&P|증시|밸류체인|공급망|산업\s*전반')
...
def _company_only_factor(f):
    ...
    name = f.get('name', '') or ''
    # 파급 대상이 이름이나 설명에 적혀 있으면 개별 기업 뉴스가 아니라 그 뉴스가
    # 업종·시장으로 번진 재료다. 설명까지 보는 이유 — 이름은 사건을 부르는 자리라
    # 짧고('구글 광고 사업 분할 기각'), 파급은 대개 설명에 적힌다.
    # 이 검사가 category 라벨보다 **먼저** 온다. 86항의 원칙이 "라벨이 아니라 글자를
    # 본다"이므로 방향도 양쪽이어야 한다 — 라벨이 시장 카테고리여도 내용이 개별
    # 기업이면 잡고, 라벨이 company_specific이어도 내용이 시장급이면 통과시킨다.
    if re.search(_G4_SPILLOVER, name + ' ' + (f.get('desc', '') or '')):
        return False
