...
def _reconcile(sym, st, label, kb, gear=None):
    ...
    88항: 차트 판독은 문장을 보탤 수 있어도 Gear 1(200일선 아래)에서 신규 진입(accumulate)을 열 수는 없다."""
    ca = _chart_of(sym)
    if not ca:
        return st, label, kb
...
def _audit_view(sym, v):
    """88항 검문. 판정(stance)이 숫자를 위반하면 내리고, 문장이 숫자와 어긋나면 경고한다.
    규칙: Gear 1 + 검증 신호(매수점수 80+ 또는 RSI 30 미만) 없음 → accumulate 금지.
          RSI 40 이상인데 '과매도'라고 단정(탈출·아님 서술 제외) → 경고.
          휴장·마감인데 '장중' → 경고(생성기는 캘린더를 쓰므로 새로 생기면 버그)."""
    n = v.get('nums') or {}
    buy, gear, rsi = n.get('buy') or 50, n.get('gear') or 2, n.get('rsi')
    sig_on = buy >= 80 or (rsi is not None and rsi < 30)
    if v.get('stance') == 'accumulate' and gear <= 1 and not sig_on:
        # 과매도 탈출 반전(tsla 경로)은 rsi5dAgo<42 를 전제로 하므로 RSI 50 미만이어야 자연스럽다
    ...
    txt = ' '.join([v.get('commentary') or ''] + [str(i) for b in (v.get('blocks') or []) for i in ([b.get('h', '')] + list(b.get('items') or []))])
    if rsi is not None and rsi >= 40 and _re.search(r'과매도', txt) and not _re.search(r'과매도\s*(구간\s*)?(탈출|아님|아니|미만|기준|신호)', txt):
        print(f'[audit] {sym}: RSI {rsi:.0f} 인데 과매도 서술: {txt[:80]!r}')
    if '장중' in txt and us_session_now() != 'open':
        print(f'[audit] {sym}: 세션 {us_session_now()} 인데 장중 서술: {txt[:80]!r}')
...
def tsla_view(s):
    ...
    if buy >= 80:
        ...
        # 88항(2026-09-08): 정찰대 조건은 '과매도 탈출' 하나다. 단순 반등 일수(_days>=2)로 열면
        ...
        if _escape:
            ...
            body_en = (f'A {_days}-day rebound, {_pv.replace("RSI ", "RSI ")}, {_devtxt.replace("200일선 대비", "")} versus the '
...
def _audience(stance, buy, sell, gear, rsi, hot, down, sig_label):
    ...
    # 신규 진입
    # 88항: 검증 신호(매수점수 80+ 또는 RSI 30 미만)가 실제로 켜졌을 때만 '발동'이라고 쓴다.
...
def main():
    ...
    # 88항 출력 검문(원칙 5·9): 숫자와 어긋나는 판정·문장은 파일에 쓰기 전에 걸린다.
