...
## [2026-09-22] 앱 페이지 세 번째 칸 — Insight Times

성동님: "https://ezlong.com/app/ 맨 위에 3번째 앱 서비스로 insightimes.com 홍보 소개해줘.
네이티브 앱은 아니지만 웹앱도 앱이니."

- `app/index.html` 에 세 번째 카드. 앞의 두 카드와 같은 틀(분류·제목·설명·화면 줄·단추)
- 화면 네 장 `app/shots/it-1~4.jpg` — 폰 폭(390, 2배)으로 맥에서 찍었다. 첫 화면 · 기사 ·
  숫자판 · 영문판. 헤드리스 크롬 기본 모드는 창을 500px 아래로 못 줄여 오른쪽이 잘렸다.
  폰 흉내(isMobile)로 다시 찍었다
- 단추: 웹진 열기 · 뉴스레터 받기(#newsletter) · English, 배지 '홈 화면에 추가하면 앱처럼'
- 구조화 데이터 ItemList 3번에 WebApplication 으로 추가, description·keywords 에 한 줄

## [2026-09-13] 반등·반락 판별 패널 — 외부 세션 제안을 검증하고 반영

### 어디서 왔나

외부 세션(Codex)이 미리보기와 인계 문서를 넘겼다. 운영 저장소 잠금을 확인하고
**사본에서만** 작업했으며 커밋·배포·모델 호출을 하지 않았다. 그 전제가 검토를 쉽게 했다.

### 왜 채택했나 — 그대로 믿지 않고 돌려 봤다

미리보기 화면이 아니라 **엔진 함수를 직접 불러 오늘 데이터 15종목을 전수 판정**했다.

- 14종목 판정, 1종목(VIX) 보류 — VIX 는 이동평균이 없어 **올바른 보류**다
- 필요한 필드가 `market-signals.json` 에 **이미 전부 있다**
  (`high20dExcl` · `low20dExcl` · `rsi5dAgo` · `hist5dAgo` · `volRatio`).
  새 파이프라인이 필요 없다 — 기존 스키마를 읽고 그 위에 지었다

설계에서 좋았던 것:

- **판정을 코드가 한다.** 모델 호출 0건 (13절)
- **지표를 독립 투표로 합산하지 않는다.** RSI·MACD·이동평균은 모두 가격에서 나온 값이라
  세 표로 더하면 같은 정보를 세 번 세는 셈이다. 구조가 상태를 게이팅하는 방식으로 피했다
- **판정 보류 상태**가 있다. 데이터 부족·시각 이상이면 판정하지 않는다
- CSS 가 14px 하한·44px 터치·포커스 표시를 다 지킨다
- 테스트 12건이 `now` 를 인자로 받아 **결정적**이다

### 반영하며 고친 것 둘

**1) 기존 판정과 어긋나면 그 사실을 말한다.**

실측에서 정면 충돌이 나왔다. IWM 이 이 엔진에서는 **"하방 이탈 · 반락 경계"** 인데
기존 화면의 매수 점수는 **72점**, Gear 3 이었다.

**버그가 아니라 철학의 차이다.** 36항의 S-CORE 는 FEAR 50% 라 공포가 클수록 점수가
오른다 — "사람들이 가장 두려워할 때가 매수 타이밍". 이 엔진은 "구조가 깨지면 기다려라"다.

둘 다 근거가 있지만 한 화면에 말없이 나란히 두면 13절 위반이다. 그래서 **숨기지 않고
설명한다** — `alignmentOf()` 가 기존 점수와 견주고, 어긋나면 그 이유를 적는다.
어느 쪽이 옳다고 정하지 않는다. 다른 것을 보고 있다고 말한다.

**2) 스냅샷이 몇 시간 전 것인지 숫자로 말한다.**

생성 시각만 적으면 '지금'처럼 읽힌다. 오늘 Market Cycle 에서 고친 것과 같은 병이라
경과 시간을 함께 적고, 24시간을 넘기면 눈에 띄게 표시한다.
96시간 문턱은 주말을 감싸는 값이라 그대로 뒀다.

### 위치

**볼카운터 바로 위, 탭 외부 고정.** 볼카운터가 이미 전 탭에 뜨는 자리라 성격이 같다.
인계 문서가 지적한 "`#swing-strategy` 진입 때도 보인다"는 문제는, 볼카운터와 같은
층에 두는 것으로 답했다.

### 검증

`node scripts/swing-reversal-selftest.js` — 109 항목, 세 부로 나눴다.

- **1부: 원안 12건을 그대로 옮긴 기준선.** 반영 과정에서 판정이 달라지지 않았음을 잠근다
- **2부: 더한 것** — IWM 조합 재현(하방 이탈 + 매수 72 → 어긋남 표시),
  돌파 + 낮은 점수, 점수 없을 때, 문턱 직전/직후, 모델 미호출, 14px 하한
- **3부: 오늘 실제 데이터 전수** — 축 넷, 방향이 −1/0/1, 지지 < 저항

브라우저 렌더 46 항목 — 라이트·다크 × 390·1280px.
볼카운터보다 위에 있는지, 종목·보유상황을 바꾸면 문구가 달라지는지까지 확인했다.

### 아직 아닌 것 (화면에도 적었다)

- 거래량 1.2배 같은 기준은 **적중률을 검증하지 않은 가정**이다.
  `scripts/backtest-base-rates.py` 가 있으니 91항대로 기저율을 붙일 수 있다
- 돌파·이탈 **실패를 시계열로 확정**하는 로직이 없다
- 종목별 "어제와 무엇이 달라졌나" — `previousSignals` 가 QQQ 만 담아 확장이 필요하다
- 다국어 대시보드 5벌에는 넣지 않았다. 한국어에서 먼저 굴려 본다

### 복구

```bash
git checkout stable-20260913-reversal -- <파일들>
```

## [2026-09-13] 포트폴리오 — 2배 레버리지 토글 제거

### 왜

'AI/반도체 공격적 수익 추구형'에 **"2배 레버리지 추가 (QLD, USD)"** 체크박스가 있었다.
운영 피드백: **국내 증권사 증거금 요건이 높아져 실제로 실행할 수 있는 경우가 거의 없어졌다.**

계산상 가능한 것과 계좌에서 실행 가능한 것은 다르다. 못 하는 선택지를 화면에 두면
그 자체가 오해를 만든다 — 방문자는 그 그림을 보고 계획을 세운다.

### 고친 것

여섯 벌(한국어 + en·ja·zh·es·pt) 전부에서 걷어냈다. 항목마다 잔존 0을 확인했다.

- `leverage` 비중표(QLD 10% · USD 8% 포함) 삭제
- `isLeveraged` 상태 · 토글 UI · `lev-chk` · `.leverage-row` / `.leverage-label` CSS 삭제
- `handleStyleSelect(styleKey, lev)` → `handleStyleSelect(styleKey)` (인자 하나로)
- `const weights = (lev && s.leverage) ? s.leverage : s.weights;` → `s.weights`
- `isAgg` 분기 삭제

공격형 비중표는 그대로 살아 있다 — SMH 24 · TIGER_KR_SEMI 11 · KODEX_AI_TECH 35 ·
KODEX_POWER 20 · KIWOOM_Q 10 = **100**.

### 함께 한 것

목록에는 여전히 레버리지 상품이 남아 있다(QLD · TQQQ · USD · SOXL). '완전 개인화'나
'종목 추가'에서 고를 수 있으므로, **한국어판에만** 같은 사실을 적었다.

- 네 상품 설명에 "국내 증권사 증거금 요건이 높아 실제 매수가 제한될 수 있습니다"
- 본문 레버리지 경고에 한 문장 — "계산상 가능한 것과 계좌에서 실행 가능한 것은 다릅니다"
- 기존 **변동성 감쇠 경고는 그대로** 뒀다. 좋은 글이고 여전히 맞는 말이다
- 다른 언어 독자는 한국 규제 대상이 아니므로 번역본에는 넣지 않았다

### 검증

`node scripts/pipeline-selftest.js` — 123 항목(기존 71 + 이번 52).
여섯 벌 각각에서 잔존 0과 비중 합계 100을 본다.

브라우저 렌더 54 항목 — 여섯 언어 × 라이트/다크 × 390/1280px.
토글 없음 · 카드 8장 유지 · 콘솔 오류 0. 공격형을 실제로 눌러 비중 24/11/35/20/10 이
그대로 들어가는 것까지 확인했다.

### 복구

```bash
git checkout stable-20260913-no-leverage -- <파일들>
```

## [2026-09-13] 절세 계좌 세법 엔진 — 규칙표로 분리

### 먼저, 내 판단이 틀렸던 것

첫 점검에서 "코드의 ISA 한도(연 2,000만원·비과세 200만원)가 옛 법일 가능성이 높다"고
적었다. **틀렸다.** 2차 자료(블로그)들이 "2026 개편 반영, 연 4,000만원·비과세 500만원"
이라고 일제히 말하고 있었는데, 그건 **8월 발표된 개편안**이었고 **9월 1일 최종안에서
철회**됐다. 코드의 숫자가 맞았다.

교훈은 분명하다 — **세법은 2차 자료로 판단하지 않는다.** 검토서에 "확인 필요"로 달아
둔 것은 다행이지만 "개연성 높음"으로 기운 것 자체가 잘못이었다. 이번에는 1차 자료
(2026년 세제개편 최종안)를 받고 나서 손댔다.

### 증상 (확인 없이도 확실했던 것들)

세율이 계산 함수 세 곳에 흩어져 박혀 있었고, 빠뜨린 규칙이 넷이었다.

1. **연금 수령 시 과세제외 원금을 안 뺐다.** 평가액 전체에 5.5%를 매겼다.
   그런데 **중도해지 경로에서는 제대로 뺐다** — 같은 파일 안에서 규칙이 엇갈렸다.
2. **연 1,500만원 초과 분리과세를 몰랐다.** 연금소득이 이 금액을 넘으면 5.5%가 아니라
   **16.5%** 다(초과분만이 아니라 전액). 은퇴 자산 4억을 20년에 나눠 받으면 연 2,000만원이라
   여기 걸리는데, 그 구간에서 **세금을 3분의 1로 축소해** 보여주고 있었다.
3. **연금소득세를 일괄 5.5% 로 봤다.** 70~79세 4.4%, 80세 이상 3.3%다.
4. **ISA 누적 1억 한도를 안 봤다.** 연 2,000만원만 자르고 누적은 무제한이었다.
   6년 넣으면 1억 2,000만원이 전부 ISA 로 계산됐다. 페이지 FAQ 는 "총 1억원 한도"라고
   적고 있었으니 **글과 계산이 어긋났다.**
5. **연금계좌 납입한도 1,800만원**을 몰랐다(세액공제 한도 900만원과 다른 값).

### 고친 것

- **`ez-tax-rules.js` 신설.** 세율·한도를 한 곳에 모으고 **적용일(`ASOF`)과 근거(`SOURCE`)**
  를 함께 적는다. 기준일 없는 세율은 시간이 지나면 조용히 거짓말이 된다.
  `pensionWithdrawalTax` · `earlyExitTax` · `isaTax` · `overseasCapitalGainTax` ·
  `pensionRateOver` 를 제공한다.
- **`PENDING` 블록** — 시행 예정 규칙은 따로 두고 `계산반영: false` 로 못 박는다.
  생산적금융 ISA(2027 시행 예정, 국회 심사 중)가 여기 들어간다. 확정 전에 숫자를 바꾸면
  틀린 법으로 틀린 법을 대체하는 셈이다.
- **`splitIsaSchedule()`** — 연 한도와 누적 한도를 함께 보고, 넘치는 몫은 직투로 흘린다.
- **연금 납입한도 적용** — 1,800만원 초과분도 직투로 흘린다.
- **화면에 가정 공개** — 20년 분할 수령 · 1,500만원 기준 · 연령별 세율 · 과세제외 원금 ·
  ISA 누적 한도 · 환급금 재투자 가정. 빠진 것(ISA 만기 연금전환 추가공제)도 함께 적었다.

### 검증

`node scripts/tax-selftest.js` — 63 항목. 두 층으로 본다.

- 1층: 규칙표가 규칙대로 계산하는가 (경계값 포함 — 연 1,500만원 **정확히**는 아직
  연금소득세, 넘기면 분리과세)
- 2층: 시뮬레이터 HTML 안의 함수를 **그대로 잘라 와** 돌린다

페이지 FAQ 가 드는 공식 예시를 테스트로 박았다 — "1,000만원 납입 + 수익 200만원 →
1,200만원에 16.5% 인 198만원". 통과한다.

시험을 만들다 배운 것 하나: 1,000만원을 납입하면 900만원만 공제 대상이라 **100만원이
과세제외 원금**이 된다. 처음에 0으로 잡았다가 코드가 맞고 시험이 틀렸음을 확인했다.

브라우저 렌더 22 항목 — 라이트·다크 × 390·1280px.

### 남긴 것

- **ISA 만기 연금전환 추가공제**(이전액의 10%, 최대 300만원)는 아직 계산에 없다.
  화면에 그렇게 적어 두었다 — 실제 결과는 화면보다 유리할 수 있다.
- **생산적금융 ISA는 12월 국회 처리 결과를 보고** 반영한다.

### 복구

```bash
git checkout stable-20260913-tax-rules -- <파일들>
```

## [2026-09-13] 차트분석 매매 레벨 검문 — 실행 가능한 폭인가

### 증상

배포된 66건을 전수 계산했다. **손익비 1 미만이 26건.** 잃을 수 있는 돈이 벌 수 있는
돈보다 큰 설정이다. `action=매수` 로 나온 것 중에도 다섯이 그랬다 — COHR 0.17 ·
MU 0.44 · MRVL 0.52 · SMH 0.59 · AAPL 0.95. 손절폭 1.5% 미만이 25건, TSLA 는 0.28%,
AMZN·INTU 는 목표가가 진입가와 같았다.

원인은 모델 등급이 아니라 **프롬프트 규칙 8** 이었다. 화면에 `$0.00` 이 뜨는 것을
막으려고 "가장 가까운 피벗을 그대로 가져다 채워라" 라고 시켰더니, 모델이 현재가에서
0.3% 떨어진 피벗을 손절에 넣었다. 방어 함수는 **0 만 잡고** 0 아닌 헛값은 통과시켰다.

파고들자 더 나온 것이 셋.

- **화면이 쓰는 필드는 따로 있었다.** 차트분석 페이지는 `entry/stop/target` 이 아니라
  `profitTarget1·profitTarget2·stopLoss` 를 그린다. 이 셋은 **어떤 검문도 받지 않았다.**
  손익비 중앙값 **0.73**, 1 미만 43건. 손절이 현재가보다 위에 찍힌 것 셋(IEF·LQD·TLT),
  1차 익절이 현재가와 같은 것 하나(CIEN), 2차 익절이 1차보다 낮은 것 셋.
  같은 판단에서 나온 두 벌의 숫자가 서로 따로 놀고 있었다(13절 위반).
- **ATR 을 계산하고 버렸다.** 산출물 `indicators` 에 안 실려서 화면도 사후 검증도
  변동성 대비 몇 배인지 볼 수 없었다.
- **저가 자산의 레벨이 뭉개졌다.** 전부 소수점 2자리로 반올림해, DOGE-USD($0.0851)는
  진입 0.09 / 손절 0.08 / 목표 0.09 가 되어 손익비를 셀 수조차 없었다.

### 고친 것

- **`validateTradeLevels()` 신설.** 방향 정합 · 손절폭(ATR 1.0~4.0배) · 손익비(1.5 이상)
  · 목표≠진입을 본다. 미달이면 모델 숫자를 버리고 ATR 기준으로 재산출하되,
  스윙 지지·저항이 허용 폭 안에 있으면 **그 레벨을 우선** 쓴다(변동성만으로 만든 선보다
  실제 지지·저항이 낫다). 보정했으면 `levelsAdjusted` 로 남긴다 — 조용히 바꾸면
  본문 문장의 숫자와 어긋나도 독자가 알 수 없다.
- **`deriveDisplayTargets()` 신설.** 익절·손절 표시값을 확정 레벨에서 항상 파생시킨다.
  손절=stop · 1차 익절=target · 2차 익절=1차의 1.75배 폭. 두 벌이 어긋날 여지를 없앴다.
- **프롬프트 규칙 8 교체.** "가장 가까운 피벗" → "ATR 1.0~4.0배, 손익비 1.5 이상,
  방향 정합". 현재 ATR 값을 프롬프트에 실어 준다.
- **`priceDecimals()`** — 가격 크기에 맞춰 2~6자리. 저가 자산에서 레벨이 뭉개지지 않는다.
- **ATR 을 산출물에 싣는다.**
- **화면에 진입가 칸 추가.** 손익비는 진입가 기준인데 화면이 진입가를 안 보여줘서,
  방문자는 현재가 기준으로 읽었다. 같은 플랜이 다른 숫자로 보이던 문제(13절).
  진입가 기준 손익비를 함께 적고, 1.5 미만이면 "실행 가치가 낮습니다"를 붙인다.
  그리드는 넷으로 늘리고 폰에서는 2×2 로 접는다.

### 검증 — 배포된 66건 전수 재계산

| 항목 | 고치기 전 | 고친 뒤 |
|---|---|---|
| 손익비 1.5 미만 (entry 기준) | 41건 | **0건** |
| 방향 불일치 | 11건 | **0건** |
| 목표 = 진입 | 3건 | **0건** |
| 표시값 손익비 중앙값 | 0.74 | **2.00** |
| 표시값 방향 이상 | 6건 | **0건** |
| 2차 익절 역전 | 3건 | **0건** |

66건 중 45건이 재산출, 21건은 그대로 통과했다.
`node scripts/pipeline-selftest.js` — 71 항목(배치 03 의 43 항목 포함).

### 정정

첫 점검 때 "이 숫자들이 atmr-dashboard 트레이드 플랜 카드에 그대로 노출된다"고 적었는데,
**대시보드에는 이미 `RR_MIN: 1.0` 게이트가 있어 손익비 1 미만이면 플랜을 싣지 않고 있었다.**
그 게이트를 못 보고 쓴 문장이다. 다만 실제 노출처는 차트분석 페이지였고 그쪽은 검문이
없었으므로, 문제 자체는 오히려 더 컸다.

### 남긴 것

새 게이트는 **생성 시점**에 돈다. 기존 `data/analysis-*.json` 은 다음 파이프라인 실행
때 갱신된다. 그 전까지 화면은 옛 값을 보여주되 손익비를 정직하게 표시하고 1.5 미만이면
경고한다. 3개월 묵은 11종목(ARM·ORCL·DDOG·CRWV·BMNR·CART·CAVA·GLBE·HYG·IEF·LQD)의
재생성은 API 키가 필요해 파이프라인 몫이다.

### 복구

```bash
git checkout stable-20260913-level-gate -- <파일들>
```

## [2026-09-13] 실패를 실패라고 말하게 한다 — 세 도구의 같은 병

### 증상

세 곳이 같은 병을 앓고 있었다. **재료가 없는 것과 시장이 건강한 것을 구분하지 않는다.**

1. **Market Cycle** — `let allAbove = true` 로 시작해 실패한 종목은 건너뛰었다.
   세 지수가 **전부 실패해도** 초록 "정상: SMA 위" 배지가 떴다.
   더 나쁜 건 코멘트다. `commentSma()` 가 `belows.length === 0` 을 보고
   **데이터가 하나도 없는데** "3개 지수 모두 30W SMA 위에서 추세 상승 중입니다.
   구조적 불안 요소는 없습니다" 라고 썼다. 배지보다 이쪽이 더 단정적이었다.
2. **Market Cycle 타임스탬프** — 상단이 `new Date()` 로 **접속 시각**을 찍었다.
   주봉은 금요일 종가로 닫히고 수집은 하루 한 번이라 며칠씩 어긋난다.
   실측: 화면은 09-13 새벽인데 데이터는 09-07 주봉이었다(6일 차이).
3. **차트분석** — 생성이 실패하면 옛 분석 문장을 보존하면서 바깥 `updatedAt` 은
   지금 시각으로 새로 찍었다. 화면은 그걸 "최종 업데이트"로 보여 준다.
4. **모델 포트폴리오** — 종목 수집이 실패하면 그 종목만 지난주 값을 유지하는데
   문서 `generatedAt` 은 최신으로 갱신됐다. 실패는 로그에만 남았다.

### 고친 것

- **판정불가 상태 신설.** `okCount`/`aboveCount` 로 센다. 성공한 종목이 하나도 없으면
  "판정 불가: 데이터 없음"(회색), 일부만 들어오면 "혼조: 일부 판정 불가"(앰버).
  초록은 **세 지수가 다 들어오고 전부 SMA 위일 때만** 나온다.
- **`commentSma()` 가드.** 0개면 재료가 없다고 말하고, 일부면 몇 개가 들어왔는지
  밝힌 뒤 "전면 판정으로 읽지 마십시오"를 붙인다.
- **타임스탬프를 데이터 기준시각으로.** `fetchWeekly` 가 `updatedAt` 과 마지막 봉
  시각을 함께 돌려주고, 화면은 **가장 뒤처진 재료**의 시각을 말한다.
  실측 결과: "마지막 주봉 2026-09-07 · 수집 2026-09-12 09:08 KST".
  열흘을 넘기면 "갱신 지연"을 스스로 붙인다.
- **브레드스 날짜 정렬.** RSP 와 SPY 의 마지막 봉을 날짜 확인 없이 각각 쓰고 있었다.
  한쪽만 먼저 갱신되면 서로 다른 4주 구간을 비교한다. 공통 주봉을 찾아 기준점으로 쓰고,
  없으면 "판정 불가: 공통 주봉 부족".
- **시세 시각과 분석 시각 분리.** `updatedAt`(파일 기록) / `analysisAt`(분석 생성).
  보존이면 `analysisAt` 은 **옛 시각 그대로** 남는다. 둘이 벌어지면 화면이
  "시세 기준: … · AI 분석: … (N일 전 판단)" 이라고 말한다.
  옛 산출물에 `analysisAt` 이 없으면 `updatedAt` 으로 대신한다(하위 호환).
- **종목별 기준일.** 성공한 종목에만 `asOf` 를 찍는다. 문서 기준일보다 사흘 이상
  뒤처지면 그 종목에만 "N일 전 값" 배지. 주간 파이프라인이라 하루 이틀은 정상이다.

### 검증

`node scripts/pipeline-selftest.js` — 43 항목.
HTML·스크립트 안의 함수를 **그대로 잘라 와** 돌린다(재구현 금지).
`pickAnalysis` 다섯 갈래, `commentSma` 세 갈래, `lagNote` 다섯 갈래.

브라우저 실패 시나리오 16 항목 — 데이터 요청을 실제로 끊어서 확인했다.

- 정상(전부 수신) : "정상: SMA 위" · "마지막 주봉 2026-09-07 · 수집 09-12 09:08 KST"
- 전멸(셋 다 차단) : "판정 불가: 데이터 없음" · "받지 못했습니다 … 판정할 재료가 없다"
- 일부(SPY만 수신) : "혼조: 일부 판정 불가" · "SPY 1개만 데이터가 들어왔습니다"
- 브레드스(SPY 없음) : "데이터 준비 중"

### 복구

```bash
git checkout stable-20260913-fail-loud -- <파일들>
```

## [2026-09-13] 계산기 네 개의 이율 환산 규칙 통일 — ez-finance.js 신설

### 증상

같은 사이트의 네 계산기가 "연 수익률 → 월 이율"을 서로 다른 규칙으로 바꾸고 있었다.

- 포트폴리오 복리 : `Math.pow(1+r, 1/12)-1` (기하환산, 맞음)
- DCA · 은퇴 역산 : `r/12` (산술환산, 틀림) — **화면에는 "CAGR"이라 적혀 있었다**
- 복리 계산기     : `r/12` (명목이율, 설계 의도) — 화면이 그 차이를 말하지 않았다

CAGR 은 기하 정의다. 라벨과 계산이 어긋났고 오차는 기간에 비례해 벌어졌다.

| 조건 | 고치기 전 | 고친 뒤 | 차이 |
|---|---|---|---|
| 원금 1억 · 연 14% · 20년 | 16.18억 | 13.74억 | 17.7% 과대 |
| 은퇴 목표 15억 · 20년 · 연 14% | 월 115만원 | 월 129만원 | 필요액 12% 과소 |
| 연 14% · 30년 | — | — | 27.7% 과대 |

### 고친 것

1. **`ez-finance.js` 신설** — 이율 환산·미래가치·역산·물가의 단일 출처.
   `monthlyFromCAGR` / `monthlyFromNominal` / `effectiveAnnual` / `fvLump` /
   `fvAnnuity` / `pmtForTarget` / `realValue` / `inflatedWithdrawal`.
   개별 파일에서 직접 나누지 않는다. 계산기를 새로 만들 때도 이 함수만 쓴다.
2. **31 줄 치환** — 한국어 + en·ja·zh·es·pt 여섯 벌, 파일 24 개.
   공유 함수는 여러 파일에 산다(8항). 한국어만 고치면 번역본은 계속 틀린 값을 낸다.
3. **환율도 단일 출처로** — `EZFin.USD_KRW` = 1342 (`USD_KRW_ASOF` 2026-09-13).
   파일마다 1380 원이 따로 박혀 있었고 기준일 표기가 넉 달 묵어 있었다.
4. **은퇴 계산기에 물가 연동 인출** (기본 켬, 연 2.5%).
   4% 룰(트리니티 연구)은 첫해 인출액을 해마다 물가만큼 올리는 것이 정의다.
   명목 고정으로 굴리면 그 이름의 근거가 이 계산에 적용되지 않는다.
5. **복리 계산기에 명목/실효 병기** — 연 14% 를 월 복리로 굴리면 실효 14.93%,
   일 복리면 15.02%. 같은 입력에 다른 답이 나오는 이유를 화면이 말한다.
6. **귀속 오류 정정** — 같은 페이지 안에서 7% 도 14% 도 "S&P500"이라 불리고 있었다.
   구조화 데이터(JSON-LD)의 "연 14% CAGR(S&P500 역사적 평균)"은 사실이 아니다 —
   S&P500 배당 포함 장기 평균은 10% 안팎이다. 14% 는 나스닥100 의 특정 구간 값이다.
   검색 결과에 인용되는 자리라 가장 널리 퍼지는 문장이 하필 틀린 문장이었다.
7. **DCA 참고 CAGR 에 구간 표기** — 최근 10년 구간의 참고치이지 장기 평균이 아니다.

### 검증

`node scripts/ez-finance-selftest.js` — 59 항목. 두 층으로 본다.

- 1층: `ez-finance.js` 가 정의대로 동작하는가 (왕복 일치·경계·0%·음수·NaN)
- 2층: **계산기 HTML 안의 함수를 그대로 잘라 와** 돌린다(재구현 금지, 90항 규칙 3과 같은 이유).
  역산 → 순방향 왕복이 목표와 맞는지, 네 계산기가 같은 조건에 같은 답을 내는지

브라우저 렌더 73 항목 — 라이트·다크 × 390·1280px, 콘솔 에러 0, 가로 넘침 0.

### 남긴 것 (다음 배치)

- 14px 미만 텍스트가 아직 많다 — 은퇴 계산기 46건, 복리 계산기 31건.
  대부분 공용 푸터의 도서 배지(11px)와 입력 라벨(12px)이다. 14항 위반이라 별도로 잡는다.
- `EZFin.USD_KRW` 는 아직 손으로 갱신한다. 주기 갱신 자동화가 필요하다.

### 복구

```bash
git checkout stable-20260913-calc-unify -- <파일들>   # forward-fix 로만 되돌린다
```
